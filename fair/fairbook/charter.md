...
**Draft, front matter, and the full-book conformance screen are complete.** Cover page graphic remains, sourced by Matt outside this project's drafting scope. The conformance screen — reference integrity, abbreviation consistency, no volatile facts in the body, TOC accuracy, Authority naming, adoption lead-ins, and single formal register per chapter — ran to completion 8/17/2026; findings fixed and verified byte-for-byte on push. `draft.md` runs clean from CC 1 through CC 10 with no legacy carry-forward material remaining; the General Rules / Code of Conduct block, the last of three legacy blocks staged since CC 4 shipped, was struck 8/17/2026 once every substantive line was confirmed re-homed into CC 2.
...
- **Abbreviations are defined in CC 1 and spelled out once more at first use in the chapters.** The first time a term appears in CC 2 through CC 10 it is written out in full with the abbreviation in parentheses, and every use after that is the abbreviation alone. A reader who skips the abbreviation list and opens their own chapter still meets the term once in full. "How to Use This Book" does not satisfy this: CC 2 spells out CSF, CSF GCR, and IAFE again on their first use there. This applies to source document names only, never to a named provision inside one of those documents, which are spelled out in full at every citation, per the HSRB citation rule above.
- **A Colorado State Fair document carries the CSF tag in its abbreviation.** GCR alone reads as a freestanding rulemaking body rather than the state fair's rulebook. The five documents whose titles begin with Colorado State Fair are abbreviated CSF GCR, CSF GEFA, CSF CCR, CSF CCGP, and CSF DTR, in CC 1 and in every use. HSRB is a Colorado 4-H book, not a state fair document, and stays HSRB.
...
Locked 8/12/2026 at CC 3.1.12 (then numbered CC 3.1.11). The county rule gave space priority in the barns to Senior members, full stop. CSF allocates by animal, one pen per head delivered, with no ranking of exhibitors at all. Custer is running out of space, so neither model survives alone: every exhibitor is guaranteed one pen, stall, or cage per species entered, seniority sets the order of assignment, and the superintendent runs both the guaranteed pass and the distribution of whatever space is left.
...
- **Letter-suffix numbering is permanent.** New rules draft in reading-order position. Where a new rule's correct slot is already occupied by shipped, numbered text, the rule takes a letter suffix: CC 2.5a between CC 2.5 and CC 2.6. That suffix is its number for the life of the book. No full-book renumber runs, at conformance or at adoption. Chosen over renumber-at-conformance, which was written when the letters were a temporary drafting device meant to be spent, and over append-only, which sacrifices reading order permanently. The convention holds because a volunteer board reprints this book without a drafter on hand, and nothing about an insert should require touching a cross-reference. Locked 9/2/2026.
