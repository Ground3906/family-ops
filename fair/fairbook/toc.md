# Custer County Fair Book — Table of Contents

Generated from the chapter and subchapter headings in `draft.md`. This file is the single source for the book's table of contents — update a heading in `draft.md`, then regenerate this list to match, rather than maintaining the two separately.

No page numbers are listed here. The Word deliverable wires every entry to its target as a bookmark and cross-reference at final assembly, per the locked hyperlinking plan in `charter.md`.

This list reflects the book as shipped through the working-group edit pass, 8/26/2026. It does not include the Addendum, which publishes as a separate document, or the legacy General Rules block staged at the bottom of `draft.md`, which is not shipped content.

---

**Front Matter**

- How to Use This Book

**CC 1 — Definitions**

- Abbreviations
- Definitions

**CC 2 — Rules and Regulations**

- All Exhibitors
- Animal Exhibitors
- Open Exhibitors

**CC 3 — Market Livestock**

- CC 3.1 — General Market Requirements
- CC 3.2 — Market Beef
- CC 3.3 — Market Swine
- CC 3.4 — Market Sheep
- CC 3.5 — Market Goat
- CC 3.6 — Market Poultry
- CC 3.7 — Market Rabbit

**CC 4 — The Livestock Sale**

- CC 4.1 — General Sale Requirements
- CC 4.2 — Sale Limits and Required Sales
- CC 4.3 — Consignment and the Sale Declaration
- CC 4.4 — Terminal Delivery and Custody
- CC 4.5 — Discipline
- CC 4.6 — Proceeds
- CC 4.7 — Warranties on Sale Animals

**CC 5 — Horse**

**CC 6 — Non-Market and Small Animal**

- CC 6.1 — General Non-Market Requirements
- CC 6.2 — Rabbit
- CC 6.3 — Poultry
- CC 6.4 — Dog
- CC 6.5 — Cat
- CC 6.6 — Llama
- CC 6.7 — Breeding and Dairy

**CC 7 — Showmanship**

- CC 7.1 — General Showmanship Rules
- CC 7.2 — Livestock Master Showmanship Contest
- CC 7.3 — Companion Animal Master Showmanship Contest

**CC 8 — Shooting Sports**

**CC 9 — Family and Consumer Sciences and General Projects**

**CC 10 — Open Division**

- CC 10.1 — General Open Division Requirements
- CC 10.2 — Baked Goods
- CC 10.3 — Food Preservation
- CC 10.4 — Decorated Baked Goods
- CC 10.5 — Floriculture
- CC 10.6 — Horticulture
- CC 10.7 — Heritage Arts
- CC 10.8 — Scrapbooking
- CC 10.9 — Sewing/Clothing
- CC 10.10 — Jewelry
- CC 10.11 — Visual Arts
- CC 10.12 — Photography
- CC 10.13 — Miscellaneous
