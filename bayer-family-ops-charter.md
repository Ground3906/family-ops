# Bayer Family Operations — Charter v2.8
...
**OneDrive archive build plan (locked 2026-06-24).** Transport is architecturally designed but not implemented. Four sessions: (1) design lock — folder structure, inbox zone, queue zone, watcher trigger rules; (2) setup build — create shared folder, share to Kalea, configure ThinkPad OneDrive sync to 2TB, smoke test; (3) receipt watcher — PowerShell automation on ThinkPad, heartbeat-wrapped, deterministic plumbing only, no AI; (4) doctrine — archive-workflow.md in repo, charter updated with actual paths. Session 1 opens on Opus.
