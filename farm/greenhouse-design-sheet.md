...
1. Continuous steel angle sill, **L4x4x0.25**, vertical leg up the bottom plate's inside face, bolted through the rail every 24 in with backing plates. Sized up from L3x3 on 2026-09-01 - a 3 in leg covered barely half the plate. The 4 in leg reaches to within 1-3/4 in of the outside framing face; shim the plate outboard of the toe. See D-4.
...
5. Sheathing full height, up to and over the rafter top edge and down to 1 in off the container roof - that full-height sheathing is the shear and does not stop at the sill. Metal siding stops at the vent sill; EPDM below it, up the sheathing, over its cut edge at the rough opening, and across the full sill top. No weather barrier drawn, matching D-2. See Detail D-4, north roof edge.
...
**North (Detail D-4, drawn 2026-09-01):** riser cap plate (raked) up through rafter bearing, sheathing over the rafter's top edge, flat-ripped (rib removed) top course of wall Pro-Panel, standard drip edge with hemmed leg over the flat course, **drip edge turns down over the riser wall face** (covers the sheathing edge and flat wall course's top edge - locked over the alternative of stopping at the panel edge). Roof panel (4 in cantilever) laps over the drip edge. Outside closure strips under the roof panel ribs at this edge - required here because this is the up-slope, windward edge and wind-driven rain can push under open ribs; not required at the south eave because that edge is shedding water outward, not receiving it. Drawn 2026-09-01 as a section through a vent bay - the only cut taken on the north wall. The crosscut settled three things this paragraph did not carry: the EPDM wrapup at the base, the flat plates raking back off the framing face, and the flap shown in the open position.
...
- **Name the sheet a member fact came from.** Before stating any dimension, member size or bearing condition, say which drawing it was read off. If the answer came from this design sheet rather than from a drawing, stop and open the drawing. Added 2026-09-02 after four errors in one session: the sill anchor read off the wrong face of the member, a ripped cap plate that does not exist, the west wall called non-bearing when its own section heading says bearing, and S-3's door opening edited on the strength of a label. Every correct answer was already inside an SVG that had been opened in that same session. The design sheet reads like an authority and the drawings read like output. The drawings are the authority.
- **Draw the condition, not the tolerance.** Stock accessories - drip edge legs, closure strips, membrane terminations, sealant - get drawn nominal and noted. They are ordered, not fabricated, and dimensioning them invents precision the supplier will ignore. Dimension what gets cut on site. Added 2026-09-01 after five separate turns were spent asking the owner to pin numbers that the crew resolves in the field.
- **The owner's hand drawing is the source, not a starting point.** Where a hand sheet shows an arrangement - orientation, which side a member sits on, how a part is drawn - it is read and followed rather than replaced with a standard detail. Added 2026-09-01: a mirrored sheet, a re-arranged box header and an invented flap assembly all came from reaching past the paper for a convention.
- **A corrected label with uncorrected geometry is worse than both being wrong.** S-3's lookout callout was fixed on 2026-08-27 and the members were left at the old spacing, so the sheet read as authoritative and agreed with S-5 in words while disagreeing in fact. When a callout is corrected, the members it describes get measured in the same pass.
- **Type size is measured against the sheet, not judged by eye.** Note text below roughly 1 percent of sheet width disappears on a phone. Enlarging the font and the canvas together changes nothing. Added 2026-09-01.
...
**Drawing set is consistent as of 2026-09-01, with one exception: D-4 is drawn north-at-left and every other sheet is north-at-right.** Flipping the rest of the set to match is an open item. All four wall elevations carry roof plane 7 ft 8-1/4 in at the south wall and peak 14 ft 7 in, all sheets are on the S-/D- numbering scheme, and every sheet that names rafter spacing says 24 in o.c.
...
| **D-4** | `D-4-north-roof-edge-detail.svg` | north roof edge |
...
**East wall framing elevation (S-3)** - `farm/greenhouse-drawings/S-3-east-wall-framing-elevation.svg`. Drawn 2026-08-24. Supersedes the legacy dimensioned east elevation PDF above. Lookout callout corrected 16 in to 24 in o.c. on 2026-08-27 - but the geometry was not corrected with it. The sheet went on drawing 14 lookouts at 16 in o.c. plus one stray 2x12 sitting 15 in below the roof plane, under a callout that read 24 in o.c. Fixed 2026-09-01 by scripted surgical edit: 10 blocks at 24 in o.c. matching S-5 station for station, stray removed, every other byte in the file untouched. **This sheet has no generator.** **Fan rough opening still carries an ASSUMED 21 in square - verify before framing.**

**North wall (riser) framing elevation (S-4)** - `farm/greenhouse-drawings/S-4-north-wall-framing-elevation.svg`. Drawn 2026-08-25. Carries the riser height stack, the four-pair vent layout, box headers with bevel-ripped cripples, the beveled plate condition, and an east orientation arrow. Two dimension chains: left off the container top, right off grade. Rewritten 2026-08-27: rafters 31 at 16 in became 21 at 24 in matching S-5, and the rafter line now sits independent of the stud line. **Riser studs remain 16 in o.c. and did not move** - all 77 stud and cripple members are byte-identical to the previous version. **This sheet has no generator** - the rafter swap was done as a scripted surgical edit rather than a full transcription, deliberately, because re-authoring 147 members was the exact failure that cost four rounds on S-1 the same day. S-2 and S-3 have no generator either - **three sheets in the set cannot be regenerated, not one.** That claim was carried wrong from 2026-08-27 until 2026-09-01. Open item.
...
**North roof edge and riser bearing (D-4)** - `farm/greenhouse-drawings/D-4-north-roof-edge-detail.svg`. Drawn 2026-09-01 as a spec-walk against current geometry, not a trace - the hand sheet predated the beveled cap plate, the box header and the 2026-08-25 sill drop. Section through a vent bay, the only cut on the north wall. Carries beveled stud tops with flat 2x6 plates laid on the bevel, the rafter plumb cut flush and bearing full width, the box header and cripples, the vent flap shown open, sheathing full height, EPDM up over the sill top, drip edge and the 4 in roof panel overhang. Generator at `generators/generate_d4.py`. **Drawn north-at-left, looking east - the opposite hand from every other sheet in the set, on the owner's call. Scale is 1-1/2 in = 1 ft rather than 3 in, so the note type reads at phone size.** Two conditions it draws that no other sheet carries: the doubled top plate rakes 1-1/16 in back off the framing face, leaving a continuous triangular void under the rafter that is air and needs no treatment; and the EPDM wraps roughly 7-1/2 in developed at the sill, up the sheathing, over its cut edge and across the full 5-1/2 in sill top.
